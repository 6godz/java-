...
public class 审核与修改薪酬标准_jpanel extends JPanel{
	public 审核与修改薪酬标准_jpanel() {
		setSize(1080, 720);
		String[][] datas = {};
		...
		JLabel l14 = new JLabel("审核/修改薪酬标准");

		JButton b1 = new JButton("刷新");// 查询按钮
		JButton b3 = new JButton("修改");// 修改按钮
		JButton b4 = new JButton("通过");// 通过按钮
		...
		b1.setBounds(100, 50, 150, 30);// 查找按钮位置
		b3.setBounds(300, 50, 150, 30);// 修改按钮位置
		b4.setBounds(500, 50, 150, 30);// 通过按钮位置
		backButton.setBounds(700,50, 150, 30);// 返回按钮位置
		...
		b1.addActionListener(new ActionListener() // 刷新按钮
		{
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (count != -1) {
					try {
						...
						System.out.print(rs);
						JOptionPane.showMessageDialog(null, "成功修改！", "提示", JOptionPane.DEFAULT_OPTION);
						dbConn.close();
					} catch (Exception ee) {
						ee.printStackTrace();
						System.out.print("连接失败");
		...
		b4.addActionListener(new ActionListener() // 通过按钮
				{
					public void actionPerformed(ActionEvent e) {
						int count = table.getSelectedRow();// 获取选中的行
						if (count != -1) {
							try {
								...
								Statement st = dbConn.createStatement();
								System.out.println("连接数据库成功");
								String 薪酬标准编号 = table.getValueAt(count, 0).toString();//获取第count行，第0列的值
								String strSQL = "update 薪酬标准表 set 标准状态='正常'  where 薪酬标准编号=" + 薪酬标准编号;
								boolean rs = st.execute(strSQL);
								System.out.print(rs);
